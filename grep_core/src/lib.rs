...
impl ExtendedRegexpMatcher {
    fn new(pattern: String) -> ExtendedRegexpMatcher {
        ExtendedRegexpMatcher {pattern: Regex::new(&pattern).unwrap(),}
    }
}
...
mod test {
	...
	fn test_extended_regexp_matcher() {
		...
		assert_eq!(false, matcher.execute("abcdef"));
	...
	fn test_match_fix_string() {
		let matcher = Matcher::new("cd".to_string(), true);
		assert_eq!(true, matcher.execute("abcdef"));
